...
unsigned char main()
{
  ...
  config_printflags->printer_bbs_output = yesNoOption("Use for BBS output",'Y');
  config_serialportflags->serial_port_baud = baudRateOption();
  config_serialportflags->serial_port_data_bits = dataBitsOption();
  config_serialportflags->serial_port_stop_bits = stopBitsOption();
  config_serialportflags->serial_port_parity = parityOption();
